...
import UpdateSalleModal from './Models/ModifierSalle';
import AddSalleModal  from "./Models/AjouterSalle";
import { GetSalles, DeleteSalle} from '../interceptors/api';
    ...
    const handleDelete = (e, id) => {
        if(window.confirm('Êtes-vous sûr ?')){
            ...
            .then((result)=>{
                ...
                const updatedSalles = salles.filter((salle) => salle.id !== id);
...
const endIndex = Math.min(startIndex + itemsPerPage, salles.length);
  ...
  <CardFooter className="pt-0">
        ...
        <div className="relative overflow-x-auto border-1 border-gray-200 rounded-t">
          ...
          className="w-1/2 rounded bg-primary-500 py-2 text-xs font-medium uppercase leading-tight text-white shadow-md transition duration-150 ease-in-out hover:bg-primary-700 hover:shadow-lg focus:bg-primary-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-primary-800 active:shadow-lg"
                  ...
                  <AddSalleModal show={addModalShow} setUpdated={setIsUpdated}
                onHide={AddModelClose}>
</AddSalleModal>
            </div>
          )}
        </div>
        <table className="w-full text-md  text-center text-gray-600 table-auto ">
        ...
        <tbody>
        {salles.slice(startIndex, endIndex).map((item, index) => {

            return(
              <tr className="bg-white border-b hover:bg-gray-200 dark:hover:bg-gray-600">
              <td className="px-6 py-1" value={item.id}>
                    {item.id}
                </td>
                <td class="px-6 py-1">
                    {item.nom}
                ...
                <td class="px-4 text-red-600 " style={{'color':'#6590D5'}}>
                <button className="focus:outline-none mx-5" onClick={event => handleDelete(event,item.id)}>
                <FontAwesomeIcon icon={faTrashCan} size="lg"/> 
                </button>
                
                <button className="focus:outline-none" onClick={event => handleUpdate(event,item.id)}>
            ...
            <UpdateSalleModal show={editModalShow} id={editStudent} setUpdated={setIsUpdated}
                              onHide={EditModelClose}></UpdateSalleModal>
